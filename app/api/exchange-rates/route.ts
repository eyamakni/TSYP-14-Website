...
export async function GET() {
  try {
    ...
    if (data.result !== "success" || !data.rates) {
      throw new Error(
        data["error-type"] ?? "Invalid exchange-rate API response"
      );
    }
    ...
    if (Object.keys(rates).length === 0) {
      throw new Error("No supported exchange rates were returned");
    }
    ...
    return NextResponse.json(
      {
        error: "Unable to retrieve the latest exchange rates.",
      },
      {
        status: 503,
      }
